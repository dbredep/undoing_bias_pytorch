...
transformed_datasets['train'] = dl.ExternalDataset(
    path_to_feature_folder=PATH_TO_FEATURES,
    fold='train',
    holdname='Caltech101')
transformed_datasets['test'] = dl.ExternalDataset(
    path_to_feature_folder=PATH_TO_FEATURES,
    fold='test',
    holdname='Caltech101')
    ...
    for phase in ['train', 'test']:
        ...
        if phase == 'train':
            ...
            with open("results/log_train", 'a') as logfile:
                ...
                if(epoch == 1):
                    logwriter.writerow(["epoch", "train_loss", "test_loss", "test_acc_vw", "test_ap_vw"])
                logwriter.writerow([epoch, last_train_loss, epoch_loss, acc/dataset_sizes[phase], ap(y_label, y_pred)])
            print ("test set accuracy visual world: ", acc / dataset_sizes[phase])
            print ("average_precision_score visual world: ", ap(y_label, y_pred))
